...
namespace Lextm.SharpSnmpLib.Mib
{
	...
	internal sealed class ObjectTree : IObjectTree
	{
		...
		// mc++ private static readonly ILog Logger = LogManager.GetLogger("Lextm.SharpSnmpLib.Mib");
		...
		public ObjectTree(ICollection<ModuleLoader> loaders) : this()
		{
			...
			// mc++ Logger.InfoFormat(CultureInfo.InvariantCulture, "{0} module files found", loaders.Count);
		...
		private void Parse(IModule module)
		{
			...
			AddNodes(module);
			// mc++ Logger.InfoFormat(CultureInfo.InvariantCulture, "{0}-ms used to assemble {1}", watch.ElapsedMilliseconds, module.Name);
			watch.Stop();
		}
		...
		private Definition CreateExtraNodes(string module, string longParent)
		{
			...
			for (int i = 1; i < content.Length; i++)
			{
				...
				else
				{
					...
					{
						// mc++ Logger.InfoFormat(CultureInfo.InvariantCulture, "ignored {0} in module {1}", longParent, module);
					}
				}
			}
		...
		public void Refresh()
		{
			// mc++ Logger.Info("loading modules started");
			Stopwatch watch = new Stopwatch();
			watch.Start();
			...
			while (current != 0)
			{
				...
				// mc++ Logger.InfoFormat(CultureInfo.InvariantCulture, "{0} pending after {1}-ms", current, watch.ElapsedMilliseconds);
				if (current == previous)
				{
					// cannot parse more
					break;
			...
			{
				// mc++ Logger.InfoFormat(CultureInfo.InvariantCulture, "{0} is parsed", loaded);
			}
			...
			foreach (MibModule module in _pendings.Values)
			{
				...
				// mc++ Logger.Info(builder.ToString());
			}

			// mc++ Logger.Info("loading modules ended");
		}
		...
		public void Import(IEnumerable<IModule> modules)
		{
			...
			foreach (MibModule module in modules)
			{
				...
				{
					// mc++ Logger.InfoFormat(CultureInfo.InvariantCulture, "{0} ignored", module.Name);
					continue;
				}
